...
void TaskLib::takePicture(AL::ALPhotoCaptureProxy photo, string fileName){
  ...
  system("mkdir /home/nao/Puppeteer/Pictures/");
...
bool TaskLib::recordVideo(AL::ALVideoRecorderProxy recorder, string fileName, bool startRec){
  if(startRec == true){
    ...
    bool overWrite = false;
    system("mkdir /home/nao/Puppeteer/Videos/");
...
bool TaskLib::moveArms(AL::ALMotionProxy motion, bool up){
  ...
  AL::ALValue angleElbow  = AL::ALValue::array(0.0f, 0.0f);

  motion.setStiffnesses(shoulders, AL::ALValue::array(1.0f, 1.0f));
  motion.setStiffnesses(elbows, AL::ALValue::array(1.0f, 1.0f));
  motion.setAngles(elbows, angleElbow, fractionMaxSpeed);
  motion.setAngles(shoulders, angles, fractionMaxSpeed);
